...
def get_base_context(assistant, org_name, summarization=""):
    return [{"role":"user", "content":f"Your name is {assistant.name} and you work as a virtual assistant for an organisation called {org_name}. \
            Your personality is described as follows: {assistant.personality_description}. What you can do is the following: You can engage in conversation \
            and help people play music via youtube. You do not currently offer any other service.{summarization} You are not to break out of character ever, do you understand your role?"}, 
            {"role":"assistant", "content":f"Yes I do. I will from now on roleplay as {assistant.name} in accordance to your request!"}]


class Conversation_machine:
    def __init__(self, client, assistant, org="GFT", max_conv_len=50, max_prompt_len=200) -> None:
        self.org = org
    ...
    def conversation_check(self): #returns "" om conversationen är slut "\nConversation ended" otherwise
        ...
        if self.max_conv_len <= len(self.context):
            ...
            summerization = summerization.choices[0].message.content
            self.reset_context(summarization=summerization)
        else:
            prompt = [{"role":"user", "content":f"In one word, yer or no, has this conversation ended:\n{conv_so_far}"}]
